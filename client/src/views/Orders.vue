<template>
  <div class="orders">
    ...
    <div v-else>
      ...
      <!-- Submitted restock orders: intentionally NOT reactive to the global FilterBar, since
           this is a historical record of orders placed under whatever filters were active at
           order time - re-filtering shouldn't hide them. -->
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">{{ t('orders.submittedOrders') }} ({{ restockOrders.length }})</h3>
        </div>
        <div v-if="restockOrders.length === 0" class="no-data">{{ t('common.noData') }}</div>
          ...
          <table class="orders-table">
            <thead>
              <tr>
                ...
                <th class="col-date">{{ t('orders.table.orderDate') }}</th>
                <th class="col-date">{{ t('orders.table.expectedDelivery') }}</th>
                <th class="col-status">{{ t('restocking.leadTime') }}</th>
                <th class="col-value">{{ t('orders.table.totalValue') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="order in restockOrders" :key="order.id">
                ...
                <td class="col-date">{{ formatDate(order.expected_delivery) }}</td>
                <td class="col-status">
                  <span class="badge info">{{ order.max_lead_time_days }} {{ t('restocking.days') }}</span>
                </td>
...
export default {
  ...
  setup() {
    ...
    // Submitted restock orders are a historical record - not reactive to the FilterBar
